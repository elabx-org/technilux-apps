/*
Advanced Blocking Plus - Technitium DNS Server App
Based on Advanced Blocking App by Shreyas Zare (shreyas@technitium.com)
...
ADVANCED BLOCKING PLUS - ENHANCED FEATURES:
- All features from the original Advanced Blocking app
- PLUS: Each client IP can map to MULTIPLE groups (rules aggregated)
- Original Advanced Blocking: Each client IP maps to ONE group (longest prefix match)
...
BLOCKING LOGIC:
  - Domain is ALLOWED if allowed in ANY of the client's groups
  - Domain is BLOCKED if blocked in ANY group AND NOT allowed in any group
  - Allow rules take priority over block rules across all groups
*/
...
namespace AdvancedBlockingPlus
{
    public sealed class App : IDnsApplication, IDnsRequestBlockingHandler
    {
        ...
        Dictionary<EndPoint, List<string>>? _localEndPointGroupMap;
        ...
        // CHANGED: Returns List<string> instead of string? for multi-group support
        private List<string> GetGroupNames(DnsDatagram request, IPEndPoint remoteEP)
        {
            List<string> groupNames = new List<string>();

            // Check local endpoint mappings first (DoH, DoT, IP endpoint)
            if ((request.Metadata is not null) && (request.Metadata.NameServer is not null))
            {
                ...
                {
                    foreach (KeyValuePair<EndPoint, List<string>> entry in _localEndPointGroupMap!)
                    {
            ...
            // Check network mappings - collect ALL matching groups (not just longest prefix)
        ...
        public async Task InitializeAsync(IDnsServer dnsServer, string config)
        {
            ...
            // CHANGED: Parse localEndPointGroupMap with array support
            if (jsonConfig.TryReadObjectAsMap("localEndPointGroupMap",
                delegate (string localEP, JsonElement jsonGroup)
                {
                    ...
                    List<string> groups = ParseGroupValue(jsonGroup);
                    return new Tuple<EndPoint, List<string>>(ep, groups);
                },
                out Dictionary<EndPoint, List<string>> localEndPointGroupMap))
            {
                _localEndPointGroupMap = localEndPointGroupMap;
            }
            else
            {
                _localEndPointGroupMap = new Dictionary<EndPoint, List<string>>();
            }

            // CHANGED: Parse networkGroupMap with array support
            _networkGroupMap = jsonConfig.ReadObjectAsMap("networkGroupMap", delegate (string network, JsonElement jsonGroup)
            {
                ...
                List<string> groups = ParseGroupValue(jsonGroup);
                return new Tuple<NetworkAddress, List<string>>(networkAddress, groups);
            });
            ...
            foreach (KeyValuePair<string, Group> group in _groups)
            {
                ...
                _dnsServer.WriteLog("Advanced Blocking Plus loaded all zones successfully for group: " + group.Key);
            }
        ...
        // Helper to parse group value which can be string or array of strings
        private static List<string> ParseGroupValue(JsonElement jsonGroup)
        {
            List<string> groups = new List<string>();

            if (jsonGroup.ValueKind == JsonValueKind.String)
            {
                // Single group as string (backward compatible)
                string? groupName = jsonGroup.GetString();
                if (!string.IsNullOrEmpty(groupName))
                    groups.Add(groupName);
            }
            else if (jsonGroup.ValueKind == JsonValueKind.Array)
            {
                // Multiple groups as array
                foreach (JsonElement item in jsonGroup.EnumerateArray())
                {
        ...
        // CHANGED: Check if domain is allowed in ANY of the client's groups
        public Task<bool> IsAllowedAsync(DnsDatagram request, IPEndPoint remoteEP)
        {
            if (!_enableBlocking)
                return Task.FromResult(false);

            List<string> groupNames = GetGroupNames(request, remoteEP);
            if (groupNames.Count == 0)
                return Task.FromResult(false);

            DnsQuestionRecord question = request.Question[0];

            // Check all groups - allowed if ANY group allows
            foreach (string groupName in groupNames)
            {
                if (_groups!.TryGetValue(groupName, out Group? group) && group.EnableBlocking)
                {
                    if (group.IsZoneAllowed(question.Name))
        ...
        // CHANGED: Check all groups - blocked if blocked in ANY group and NOT allowed in any group
        public Task<DnsDatagram?> ProcessRequestAsync(DnsDatagram request, IPEndPoint remoteEP)
        {
            ...
            // First, check if domain is allowed in ANY group (allow takes priority)
            foreach (string groupName in groupNames)
            {
                if (_groups!.TryGetValue(groupName, out Group? group) && group.EnableBlocking)
                {
                    if (group.IsZoneAllowed(question.Name))
                        return Task.FromResult<DnsDatagram?>(null); // Allowed - don't block
                }
            }

            // Then, check if domain is blocked in ANY group
            Group? blockingGroup = null;
            string? blockedDomain = null;
            string? blockedRegex = null;
            UrlEntry? blockListUrl = null;

            foreach (string groupName in groupNames)
            {
                if (_groups!.TryGetValue(groupName, out Group? group) && group.EnableBlocking)
                {
                    if (group.IsZoneBlocked(question.Name, out blockedDomain, out blockedRegex, out blockListUrl))
                    {
                        blockingGroup = group;
                        break; // Use first blocking group for response settings
                    }
                }
            }

            if (blockingGroup is null)
                return Task.FromResult<DnsDatagram?>(null);

            string GetBlockingReport()
            {
                string blockingReport = "source=advanced-blocking-plus; group=" + blockingGroup.Name + "; groups=" + string.Join(",", groupNames);
            ...
            if (blockingGroup.AllowTxtBlockingReport && (question.Type == DnsResourceRecordType.TXT))
            {
                //return meta data
            ...
            else
            {
                ...
                if (blockingGroup.AllowTxtBlockingReport && (request.EDNS is not null))
                {
                    string blockingReport = GetBlockingReport();
        ...
        { get { return "Enhanced version of Advanced Blocking with all original features PLUS support for assigning MULTIPLE groups to each client IP address or subnet. Allows rules from different groups to be aggregated for the same client."; } }
        ...
        abstract class ListBase
        {
            ...
            private async Task<bool> DownloadListFileAsync()
            {
                try
                {
                    _dnsServer.WriteLog("Advanced Blocking Plus is downloading " + (_isAdblockList ? "adblock" : (_isRegexList ? "regex " : "") + (_isAllowList ? "allow" : "block")) + " list: " + _listUrl.AbsoluteUri);
                    ...
                    if (_listUrl.IsFile)
                    {
                        if (File.Exists(_listFilePath))
                        {
                            ...
                            {
                                _dnsServer.WriteLog("Advanced Blocking Plus successfully checked for a new update of the " + (_isAdblockList ? "adblock" : (_isRegexList ? "regex " : "") + (_isAllowList ? "allow" : "block")) + " list: " + _listUrl.AbsoluteUri);
                                return false;
                            }
                        }
                        ...
                        _dnsServer.WriteLog("Advanced Blocking Plus successfully downloaded " + (_isAdblockList ? "adblock" : (_isRegexList ? "regex " : "") + (_isAllowList ? "allow" : "block")) + " list (" + WebUtilities.GetFormattedSize(new FileInfo(_listFilePath).Length) + "): " + _listUrl.AbsoluteUri);
                        return true;
                    }
                    else
                    {
                ...
                {
                    _dnsServer.WriteLog("Advanced Blocking Plus failed to download " + (_isAdblockList ? "adblock" : (_isRegexList ? "regex " : "") + (_isAllowList ? "allow" : "block")) + " list and will use previously downloaded file (if available): " + _listUrl.AbsoluteUri + "\r\n" + ex.ToString());
                    return false;
                }
            }
        ...
        class BlockList : ListBase
        {
            ...
            private Queue<string> ReadListFile()
            {
                ...
                try
                {
                    _dnsServer.WriteLog("Advanced Blocking Plus is reading " + (_isAllowList ? "allow" : "block") + " list from: " + _listUrl.AbsoluteUri);
                    ...
                    _dnsServer.WriteLog("Advanced Blocking Plus read " + (_isAllowList ? "allow" : "block") + " list file (" + domains.Count + " domains) from: " + _listUrl.AbsoluteUri);
                }
                catch (Exception ex)
                {
                    _dnsServer.WriteLog("Advanced Blocking Plus failed to read " + (_isAllowList ? "allow" : "block") + " list from: " + _listUrl.AbsoluteUri + "\r\n" + ex.ToString());
                }
        ...
        class RegexList : ListBase
        {
            ...
            private Queue<string> ReadRegexListFile()
            {
                ...
                try
                {
                    _dnsServer.WriteLog("Advanced Blocking Plus is reading regex " + (_isAllowList ? "allow" : "block") + " list from: " + _listUrl.AbsoluteUri);
                    ...
                    _dnsServer.WriteLog("Advanced Blocking Plus read regex " + (_isAllowList ? "allow" : "block") + " list file (" + regices.Count + " regex patterns) from: " + _listUrl.AbsoluteUri);
                }
                catch (Exception ex)
                {
                    _dnsServer.WriteLog("Advanced Blocking Plus failed to read regex " + (_isAllowList ? "allow" : "block") + " list from: " + _listUrl.AbsoluteUri + "\r\n" + ex.ToString());
                }
        ...
        class AdBlockList : ListBase
        {
            ...
            private void ReadAdblockListFile(out Queue<string> allowedDomains, out Queue<string> blockedDomains)
            {
                ...
                try
                {
                    _dnsServer.WriteLog("Advanced Blocking Plus is reading adblock list from: " + _listUrl.AbsoluteUri);
                    ...
                    _dnsServer.WriteLog("Advanced Blocking Plus read adblock list file (" + (allowedDomains.Count + blockedDomains.Count) + " domains) from: " + _listUrl.AbsoluteUri);
                }
                catch (Exception ex)
                {
                    _dnsServer.WriteLog("Advanced Blocking Plus failed to read adblock list from: " + _listUrl.AbsoluteUri + "\r\n" + ex.ToString());
                }
            }
